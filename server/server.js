  ...
  try {
    const response = await fetch("https://open.tiktokapis.com/v2/oauth/token/", {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: redirect_uri || process.env.TIKTOK_REDIRECT_URI, code_verifier
      }).toString()
    });
    const data = await response.json();
    if (!response.ok || data.error) return res.status(400).json({ error: data.error || "Token exchange failed", details: data });
    res.json({ access_token: data.access_token, open_id: data.open_id, expires_in: data.expires_in, refresh_token: data.refresh_token, scope: data.scope });
  } catch (err) { res.status(500).json({ error: "Token exchange failed", details: err.message }); }
});
...
app.post("/tiktok/profile", async (req, res) => {
  ...
  try {
    ...
    res.json(data);
  } catch (err) { res.status(500).json({ error: "Profile fetch failed", details: err.message }); }
});
...
app.post("/tiktok/videos", async (req, res) => {
  ...
  if (!access_token) return res.status(400).json({ error: "Missing access_token" });
  try {
    const response = await fetch("https://open.tiktokapis.com/v2/video/list/?fields=id,title,cover_image_url,video_description,duration,like_count,comment_count,share_count,view_count",
      { method: "POST", headers: { "Authorization": "Bearer " + access_token, "Content-Type": "application/json" }, body: JSON.stringify({ max_count: max_count || 10 }) });
    const data = await response.json();
    if (!response.ok || data.error) return res.status(400).json({ error: "Failed to fetch videos", details: data });
    res.json(data);
  } catch (err) { res.status(500).json({ error: "Videos fetch failed", details: err.message }); }
});
...
app.post("/tiktok/publish", async (req, res) => {
  ...
  } catch (err) { res.status(500).json({ error: "Publish failed", details: err.message }); }
});
...
app.post("/tiktok/share", async (req, res) => {
  ...
  if (!access_token || !video_url) return res.status(400).json({ error: "Missing access_token or video_url" });
  try {
    const verify = await fetch("https://open.tiktokapis.com/v2/user/info/?fields=open_id", { headers: { "Authorization": "Bearer " + access_token } });
  ...
  } catch (err) { res.status(500).json({ error: "Share failed", details: err.message }); }
});
...
app.post("/tiktok/refresh", async (req, res) => {
  ...
  try {
    const response = await fetch("https://open.tiktokapis.com/v2/oauth/token/", {
      ...
      body: new URLSearchParams({ client_key: process.env.TIKTOK_CLIENT_KEY, client_secret: process.env.TIKTOK_CLIENT_SECRET, grant_type: "refresh_token", refresh_token }).toString()
    });
    const data = await response.json();
  ...
  } catch (err) { res.status(500).json({ error: "Token refresh failed", details: err.message }); }
});
...
app.post("/instagram/token", async (req, res) => {
  ...
  try {
    const shortRes  = await fetch("https://api.instagram.com/oauth/access_token", {
      method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams({ client_id: process.env.INSTAGRAM_APP_ID, client_secret: process.env.INSTAGRAM_APP_SECRET, grant_type: "authorization_code", redirect_uri: redirect_uri || process.env.INSTAGRAM_REDIRECT_URI, code }).toString()
    });
...
app.post("/instagram/media", async (req, res) => {
  ...
  try {
    ...
    res.json(data);
  } catch (err) { res.status(500).json({ error: "Media fetch failed", details: err.message }); }
});
...
app.post("/instagram/insights", async (req, res) => {
  ...
  try {
    ...
    res.json(data);
  } catch (err) { res.status(500).json({ error: "Insights fetch failed", details: err.message }); }
});
...
/* ================================================================
   GOOGLE TRENDS PROXY
   No API key needed — uses public Google Trends RSS feed
================================================================ */
